...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiCalendar, FiMapPin } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<RentalAgencyInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/rental-agencies` : null),
    () => getRentalAgencies({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Rental Agency',
      path: isTenantUser ? `/rental-agencies/view/${data?.[0]?.id}` : '/rental-agencies',
