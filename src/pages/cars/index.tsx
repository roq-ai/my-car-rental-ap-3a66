...
function CarListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CarInterface[]>(
    ...
    () =>
      getCars({
        relations: ['rental_agency', 'rental.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  <Th>year</Th>
                  <Th>availability</Th>
                  {hasAccess('rental_agency', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>rental_agency</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('rental_agency', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/rental-agencies/view/${record.rental_agency?.id}`}>
                          {record.rental_agency?.name}
                        </Link>
                      </Td>
                    )}
